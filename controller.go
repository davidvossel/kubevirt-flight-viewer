...
func (c *Controller) updateInFlightOperationStatus(ctx context.Context, inflightOperation *flightviewerv1alpha1.InFlightOperation) error {
	/*
		inflightOperationCopy := inflightOperation.DeepCopy()
		_, err := c.flightviewerclientset.KubevirtflightviewerV1alpha1().InFlightOperations(inflightOperation.Namespace).UpdateStatus(ctx, inflightOperationCopy, metav1.UpdateOptions{FieldManager: FieldManager})
	*/
	return nil
}
...
// to find the InFlightOperation resource that 'owns' it. It does this by looking at the
// objects metadata.ownerReferences field for an appropriate OwnerReference.
// It then enqueues that InFlightOperation resource to be processed. If the object does not
...
func (c *Controller) handleObject(obj interface{}) {
	...
	if ownerRef := metav1.GetControllerOf(object); ownerRef != nil {
		// If this object is not owned by a InFlightOperation, we should not do anything more
		// with it.
		if ownerRef.Kind != "InFlightOperation" {
			return
		}

		inflightOperation, err := c.inflightOperationsLister.InFlightOperations(object.GetNamespace()).Get(ownerRef.Name)
		if err != nil {
			logger.V(4).Info("Ignore orphaned object", "object", klog.KObj(object), "inflightOperation", ownerRef.Name)
			return
		}

		c.enqueueInFlightOperation(inflightOperation)
		return
	}
}
